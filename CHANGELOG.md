...
- Глобальный плоский `AGENTS.md` требует repository preflight перед предметной работой и после возобновления задачи; ненулевой код останавливает работу, а исключение из подтверждения ограничено clone, `fetch --all --prune` и `pull --ff-only`.
